(function(window, document) {
    ...
    handwriting.recognize = function(trace, options, callback) {
        if(window.axis.length){
            let request = window.axis;
            let language = '';
            window.object.push(request)
            // window.object = window.object.concat(window.axis)
            document.getElementById("handwriting").innerHTML='';
            // $('#handwriting-of-ul > li').remove();
            // var postArray = window.axis.map(function (inkObj) {
            //     return [inkObj[0], inkObj[1], inkObj[2]];
            // });
            ...
            xhr.open("POST", "https://inputtools.google.com/request?ime=handwriting&app="+param+"&dbg=1&cs=1&oe=UTF-8");
