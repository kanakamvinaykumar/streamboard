...
(function () { //Code isolation
	...
	function startLine(x, y, evt) {
		...
		//alert(pencilTool.secondary.active)
		Tools.drawAndSend({
			'type': 'line',
			'id': curLineId,
			'color': (pencilTool.secondary.active ? "#ffffff" : Tools.getColor()),
			'size': Tools.getSize(),
			'opacity': (pencilTool.secondary.active ? 0 : Tools.getOpacity()),
		});
	...
	function createLine(lineData) {
		...
		line.id = lineData.id;
		if((window.Type == 'handwrite') || (window.Type == 'magic')) {
			window.lineHistory.push(line.id)
		}
