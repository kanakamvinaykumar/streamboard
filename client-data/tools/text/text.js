...
input.style.height = "100px";
	...
	function startEdit() {
		$("#ctxmenu div").each(function(index) {
		...
		if (x + 250 > clientW) {
			...
			var obj = profileData.data.user.school[0];
			if(obj.isImmersiveReaderEnable) {
			  // evt.preventDefault()
			  let menu = document.createElement("div")
			  menu.id = "ctxmenu"
			  menu.style.left = (x+240) + 'px';
			  menu.style.top = curText.y * Tools.scale - document.documentElement.scrollTop+5 + 'px';
			  // menu.onmouseleave = () => ctxmenu.outerHTML = ''
			  // let icon = window.app_url+'images/icon.svg';
			  let icon = 'http://65.108.95.12:3000/images/icon.svg';
			  menu.innerHTML = "<div onclick='immersiveReader()' style='cursor:pointer;' ><img src='"+icon+"' width='50px'/></div>"
			  document.body.appendChild(menu)				
			}
		input.style.left = x + 'px';
		input.style.top = curText.y * Tools.scale - document.documentElement.scrollTop + 40 + 'px';
		// oncontextmenu = (e) => {
			// var myElement = document.getElementById('textToolInput')
	...
	function createTextField_width(fieldData) {
	...
	function createTextField(fieldData) {
		...
		var divelem = Tools.createSVGElement("div");
		elem.id = fieldData.id;
		elem.setAttribute("x", fieldData.x);
		elem.setAttribute("y", fieldData.y);
		elem.setAttribute("font-size", fieldData.size);
		...
		elem.setAttribute("width", input.style.width);
		// elem.setAttribute("fill", fieldData.color);
		elem.setAttribute("style", "color: "+fieldData.color+";font: "+fieldData.size+";height: 100%;overflow: auto")
		// elem.setAttribute("opacity", Math.max(0.1, Math.min(1, fieldData.opacity)) || 1);
		divelem.setAttribute("xmlns", "http://www.w3.org/1999/xhtml");
		if (fieldData.txt) divelem.textContent = fieldData.txt;
