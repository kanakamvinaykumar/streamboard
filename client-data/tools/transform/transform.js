...
(function transform() { //Code isolation
	...
	function onQuit(){
		if(Tools.list["Measurement"]){
			Tools.list["Measurement"].resize("small")
		}
		...
		if (performance.now() - lastTime > 70 || end) {
			...
			if(shape){
				...
				if(Array.isArray(shape.matrix)){
					...
					if(Tools.list["Measurement"]){
						Tools.list["Measurement"].updateTransform(shape)
					}
				}else{
					msg.transform=shape.matrix;
					if(Tools.list["Measurement"]){
						Tools.list["Measurement"].updateTransform()
					}
				}
